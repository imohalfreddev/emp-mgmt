...
const styles = (dark, isMobile) => ({
  ...
  card: {
    ...
    padding: isMobile ? '60px 24px' : '40px 36px',
    width: '100%',
    maxWidth: isMobile ? '100%' : '400px', // Restricts size on PC
    minHeight: isMobile ? '100vh' : 'auto', // Full height only on mobile
    boxShadow: isMobile ? 'none' : (dark ? '0 20px 60px rgba(0,0,0,0.4)' : '0 8px 40px rgba(0,0,0,0.08)'),
    boxSizing: 'border-box',
    display: 'flex',
  ...
  logoIcon: { fontSize: isMobile ? 32 : 24, color: '#f1c40f' },
  logoText: { fontSize: isMobile ? 24 : 20, fontWeight: 700, color: dark ? '#fff' : '#1a1a2e', letterSpacing: '-0.5px' },
  title: { margin: '0 0 4px', fontSize: isMobile ? 28 : 24, fontWeight: 700, color: dark ? '#f0f0f5' : '#1a1a2e', letterSpacing: '-0.5px' },
  sub: { margin: '0 0 28px', color: dark ? '#888' : '#777', fontSize: isMobile ? 16 : 14 },
  ...
  switchText: { textAlign: 'center', marginTop: isMobile ? '40px' : '20px', fontSize: 13, color: dark ? '#888' : '#777' },
